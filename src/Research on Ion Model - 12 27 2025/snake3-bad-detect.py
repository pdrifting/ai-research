...
# =================================================================
# 3. PARALLEL EXECUTION WRAPPER
# =================================================================

def gpu_worker(gpu_id, samples_per_gpu, total_bits, batch_size):
    device = torch.device(f"cuda:{gpu_id}")
    model = ChaosEngine().to(device)
    model.eval()

    results = []
    for s in range(samples_per_gpu):
        ...
        out = f"GPU[{gpu_id}] | Str {s+1:<2} | {dens:<6.3f} | {r_p:<8.3f} | {l_p:<8.3f} | {o_p:<8.3f} | {status}"
        print(out)
        results.append(passed)
    return results

if __name__ == "__main__":
    # Total samples: 21 samples across 3 GPUs (7 each), repeated in 30 runs? 
    # Interpreting "21 samples of 30 runs" as 630 total streams or 21 batches of 30.
    # To keep it manageable for data collection: 3 GPUs, each running 7 samples x 30 iterations.
    ...
    print(f"[*] Starting Stress Test on {num_gpus} GPUs...")
    print(f"[*] Goal: Detect 'Bad Mode' across {num_gpus * samples_per_gpu * iterations} total streams.")
    print(f"{'Source':<7} | {'Str':<6} | {'Dens':<6} | {'Rank P':<8} | {'LinC P':<8} | {'OverP':<8} | {'Status'}")
    print("-" * 85)

    for run in range(iterations):
        processes = []
        print(f"\n--- Global Run {run+1}/{iterations} ---")
        for g in range(num_gpus):
            p = mp.Process(target=gpu_worker, args=(g, samples_per_gpu, 1000000, 256))
            p.start()
            processes.append(p)
