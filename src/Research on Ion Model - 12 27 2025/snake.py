...
# =================================================================
# 1. THE MUTATING CHAOS ENGINE (WIDER HIDDEN DIM + MUTATION)
# =================================================================
...
class DriftLayer(nn.Module):
    def __init__(self, in_dim, out_dim, drift_magnitude=0.3):
        super().__init__()
        # Increased variance to prevent Rank collapse
    ...
    def forward(self, x):
        # Neural Drift: resample synapses
...
class ChaosEngine(nn.Module):
    def __init__(self, input_dim=512, hidden_dim=2048): # Increased hidden_dim for Rank depth
        super().__init__()
    ...
    def forward(self, x, mutate=False):
        # Inject mutation pulse if system stalls
        if mutate:
...
# =================================================================
# 3. EXECUTION WITH PULSE MUTATION
# =================================================================
...
def run_evaluation(num_streams=5, total_bits=1000000, batch_size=256):
    ...
    print(f"[*] Engine: {device} | Stochastic Mutation Mode")
    ...
    for s in range(num_streams):
        ...
        for i in range(0, total_bits, batch_size):
            # Mutate latent space every 100k bits to break Rank local minima
            mutate = (i % 100000 == 0)
            
            with torch.no_grad():
                probs = model(z, mutate=mutate)
                z = torch.roll(z, 1, dims=1)
                z += (probs - 0.5) * 0.2 + torch.randn_like(z) * 0.02
        ...
        passed = all(p >= 0.01 for p in [r_p, l_p, o_p]) and 0.48 < dens < 0.52
