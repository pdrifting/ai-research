...
void createLogicalDevice(VulkanApp* app) {
    debugPrint("Entering createLogicalDevice.");
    ...
    queueFamilyCount = sizeof(uniqueQueueFamilies) / sizeof(uniqueQueueFamilies[0]);
    ...
    printf("[DEBUG] Creating logical device with %d queue(s): %d %s\n",
       queueFamilyCount,
       uniqueQueueFamilies[0],
       queueFamilyCount == 2 ? "and second queue" : "(single queue)");
...
void createSyncObjects(VulkanApp* app) {
    ...
    VkResult result;
    result = vkCreateSemaphore(app->device, &semaphoreInfo, NULL, &app->imageAvailableSemaphore);
    checkVkResult(result, "vkCreateSemaphore (imageAvailable)");
    
    result = vkCreateSemaphore(app->device, &semaphoreInfo, NULL, &app->renderFinishedSemaphore);
    checkVkResult(result, "vkCreateSemaphore (renderFinished)");
...
void drawFrame(VulkanApp* app) {
    printf("[DEBUG] Waiting for fence...\n");
    VkResult result = vkWaitForFences(app->device, 1, &app->inFlightFence, VK_TRUE, UINT64_MAX);
    checkVkResult(result, "vkWaitForFences");
    
    result = vkResetFences(app->device, 1, &app->inFlightFence);
    checkVkResult(result, "vkResetFences");
    
    uint32_t imageIndex;
    printf("[DEBUG] Acquiring next image...\n");
    ...
    printf("[DEBUG] Resetting command buffer...\n");
    result = vkResetCommandBuffer(app->commandBuffer, 0);
    checkVkResult(result, "vkResetCommandBuffer");
    
    printf("[DEBUG] Recording command buffer...\n");
    ...
    printf("[DEBUG] Submitting to graphics queue...\n");
